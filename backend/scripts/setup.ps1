...
function Start-Database {
    Write-Step "Iniciando contenedor de PostgreSQL..."
    ...
    $containerExists = docker ps -a --format "{{.Names}}" | Select-String -Pattern "olaspp_postgres" -Quiet

    if ($containerExists) {
        Write-Warning "El contenedor ya existe"

        # Verificar si está corriendo
        $containerRunning = docker ps --format "{{.Names}}" | Select-String -Pattern "olaspp_postgres" -Quiet
...
function Wait-ForDatabase {
    ...
    while ($attempt -lt $maxAttempts) {
        try {
            docker exec olaspp_postgres pg_isready -U postgres 2>&1 | Out-Null
...
function Test-DatabaseConnection {
    ...
    try {
        docker exec olaspp_postgres psql -U postgres -d olaspp -c "SELECT 1" 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) {
...
function Show-Info {
    ...
    Write-Host "  • Base de Datos: PostgreSQL 16 + PostGIS"
    Write-Host "  • Puerto DB: 5432"
    Write-Host ""
    ...
    Write-Host "  • Ver logs de DB: docker-compose logs -f"
    Write-Host "  • Detener DB: npm run db:stop"
